    ...
    public String toString() {
        ...
        for (int i = 0; i < residues.length; i++) {
            ...
            if (residues[i] instanceof ModifiedResidue){
                sb.append(residues[i].getName());
            }
//            if (residues[i].getName().contains("(")){
//                sb.append(residues[i].getName());
//            }
            else {
              sb.append(residues[i].getFASTAChar());
            }
        }
        return sb.toString();
    }
...
//        residues = new Residue[0];
    ...
    public Peptide replace(int index, Residue replaceWith){
        // get the existing residues
        Residue[] res = this.getResidues();
        //replace with the new residue
        res[0] = replaceWith;

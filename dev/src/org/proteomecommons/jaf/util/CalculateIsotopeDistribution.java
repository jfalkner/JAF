...
public class CalculateIsotopeDistribution {
	...
	/**
	 * Sets the increment of approximation points, e.g. how many bins are
	 * created for unique isotopes. If you are deisotoping, 1 works as a pretty
	 * good value. If you are trying to find all the isotopes possible, set this
	 * value to as high as you like.
	 * 
	 * @param increment
	 */
	...
	public double[] approximateIsotopeDistribution(double massInDaltons) {
		...
		if (averageAtoms == null) {
			synchronized (this) {
				// hash for the residues
				Hashtable hash = new Hashtable();
				Residue[] commonResidues = GenericResidue.getCommonResidues();
				for (int i = 0; i < commonResidues.length; i++) {
					Atom[] as = commonResidues[i].getAtoms();
					for (int j = 0; j < as.length; j++) {
						// check if it has been seen
						Integer in = (Integer) hash.get(as[j]);
						// if not, make a new int
	...
	private void updateDist(double mass, double probability) {
		...
		for (int i = 0; i < dist.length; i++) {
			...
			if (Math.abs(i * increment + monoisotopic - mass) <= increment/2) {
				dist[i] += probability;
	...
	public static void main(String[] args) {
		...
		cid.approximateIsotopeDistribution(1444);
